...
Galaxy morphology has been a facet of study for nearly 100 years. The visual structure of 
galaxies provides clues into the nature of galactic evolution and history. Galaxy morphology
remains just as important today with new and rare galaxy types that probe various
avenues of galactic evolution, from the discovery of the green peas to ... 
These diverse galaxy types are 
Several studies utilizing visual classifications have lead to advanced understanding of the 
evolution of galaxies: bar fractions, merger rates, etc. 

Upcoming surveys like LSST and Euclid will be coming online in the next five years. 
...
evolution of these galaxies, methodologies must be put in place to handle the vast
nature of the data. Standard visual morphology methods will be unable to cope with 
the size and current automated morphologies provide only statistical morphological structure
with large uncertaintie and impurities. Methods must be developed which can efficiently 
and accurately quantify and qualify that nature of galaxy morphology. 

 In this work we present the first system which combines both human and machine
intelligence to the task of galaxy morphology. We demonstrate this through a 
re-analysis of visual galaxy morphology classifications collected during the Galaxy Zoo 2
project. We combine these classifications with a suite of non-parametric 
morphology indicators typically used for automated morphologies. Our method 
provides an order of magnitude improvement in the rate of galaxy morphology classifications. 



\textbf{Old Stuff vvv}

The age of Big Data is upon us. The astrophysics community has been  shifting focus, 
preparing for the ways in which both science and how we perform that science will change. 
Several stalwart astronomical data repositories have begun integrating analytical
tools at the source, most notably CasJobs, because downloading data will become a thing 
of the past when these data are terabytes in size. Advances in data storage, 
telescope reduction pipelines, and data access are all required. 

In addition to how we manage and compute on Big Data, we must also prepare
for how we identify objects in the sky.  We will have access to more sky, both in terms 
of area and depth, than ever before. Once those data have been reduced  and stored, 
we must find efficient, clever ways of identifying transients, radio relics, gravitational lenses, 
galaxy morphologies, and more, all while allowing for an unprecedented amount of new
serendipitous discoveries. If not,  LSST, Euclid, WFIRST will overwhelm us. 

In this paper we consider the particular problem of galaxy morphology. This 
challenge is actually several combined because it encapsulates the need to 
...
considering the data we currently have. The imaging techniques of future surveys will 
change mostly in resolution and depth; things we can account for. 
Another reason galaxy morphology is a useful example is that we can draw on 
vast, well-established citizen science projects which have contributed to several 
past publications and have lead to serendipitous discovery on multiple occasions. 
There is no doubt that to spurn this resource would be a disservice to science. 

Of course, galaxy morphology has drawbacks as well. The community firmly clings to 
a classification system developed nearly 100 years ago which works well only for 
galaxies in the local universe.  However, the endeavor has merit as understanding 
the nature of galactic structure provides us with a means to disentangle galactic 
evolution.  
...
In this paper we present a framework for combining human and machine intelligence 
for morphology classification tasks. This idea is not new in computer science circles as
several studies have been done, many on Galaxy Zoo data 
\textbf{[Microsoft research guy, Chris: citations, plz.]} 
However, these studies are generally unknown to the astrophysics community and
are, in some cases, impractical. The novelty of our approach lies in its
practicality, simplicity, and in our ability to explore the repercussions of our system by 
simulating various outcomes on previously collected data. 

We draw on visual classifications from the Galaxy Zoo project. We implement an 
algorithm first developed for the Space Warps Zooniverse project and demonstrate
for the first time that this algorithm is well suited to galaxy morphology classification
and significantly increases the rate of classification compared to Galaxy Zoo. 
We measure standard galaxy morphology indicators and use these as a feature
vector for an implementation of a Random Forest machine learning algorithm. 
We develop a decision engine with dictates how human and machine classifications 
are combined and show that our approach achieves a full order of magnitude increase
in the classification rate without sacrificing the quality of those classifications. 
We begin with an overview of the Galaxy Zoo Express framework. 
