...
numbers as a shortcut for the savvy reader. We note here that transparent portions
 of the schematic represent areas of future work. 
...
We perform a full GZX simulation incorporating both SWAP and the machine using the
 fiducial SWAP run discussed in section~\ref{sec: fiducial}. 
The machine attempts its first training on Day XXX of the run with an initial training
sample of XXX.  The machine undergos several additional nights of training, each time
with a larger training sample. 
By Day XXX, the machine's \textit{agent} has assessed that the machine is suitably trained. 
At this point, SWAP has now provided XXX subjects as a training sample. 
The machine predicts classes for the remaining XXX GZ2 subjects. 
Of those, the machine strongly predicts classes for nearly XXX subjects, thus dramatically 
increasing the overall sample of retired subjects. 
As the simulation progresses, retirement by both SWAP and
the machine tapers off. We end the simulation after XX days, having sufficiently 
...
The bottom panel of Figure~\ref{fig: money} shows the power of GZX (red) 
compared to SWAP only (light blue) and GZ2 subject retirement (dark blue). 
By dynamically generating a training sample
through a more sophisticated analysis of human classifications coupled with a 
simple feedback mechanism, we are able to retire over 200K GZ2 subjects in 2X days.  
SWAP alone retires as many subjects in 6-7X days. GZ2 took XX \textit{months} to retire
as many and 14 months to classify the entire catalog of 285K subjects. 
...
SWAP-only and GZX, again using the GZ2 labels for comparison.  
Accuracy for the combined system remains around 95\%, while
...
allow SWAP to handle the entirety of the sample, each yielding purity of ~85\% 
for a similar-sized subject set. 
Instead we make a small sacrifice in the completeness of the~\feat~sample
when incorporating the machine. Whereas SWAP alone provides nearly perfect
completeness, the machine cannot perform at that level for reasons we discuss below.
...
\subsection{Verifying our Decision Engine}
...
\subsection{Machine mis-classifications}
