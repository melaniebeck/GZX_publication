...
\documentclass[twocolumn,  ]{aastex6}%, linenumbers, trackchanges,
...
\replaced{In this paper}{As a proof of concept,} we focus on the first question of the Galaxy Zoo decision tree \added{, however we briefly discuss how such a method can be implemented for more complex tasks}. We demonstrate that our \replaced{method}{current implementation} provides a factor of 11.4 increase in the rate of galaxy morphology classification while maintaining at least 93.5\% classification accuracy as compared to Galaxy Zoo 2 published data. We first present an overview of our framework, which also serves as a blueprint for this paper. 
...
classified by eye into T-Types. We generate expert labels for these galaxies that are consistent with the labels we defined for GZ2 classifications. These are obtained
...
\begin{figure}[t!] 
\centering
\includegraphics[width=3.2in]{confusionmatrix.pdf}
...
%%%-------------------------------------------------------
\begin{figure*}[t!]
\includegraphics[width=7in]{thingy2.pdf}
\caption{\added{SWAP's intelligent retirement mechanism allows it to use 30\% fewer classifications than GZ2 for the top-level question due to SWAP's ability to retire easier subjects quickly, while more difficult subjects remain in the system to acquire additional classifications. 
...
GZ2 subjects in just 27 days.  \replaced{Visual classification through SWAP alone retires as many in 50 days, while GZ2 requires a full year. Though our analysis considers only the top-level task of GZ2's decision tree, GZX suggests a tantalizing potential to increase the classification rate by an order of magnitude or more over the traditional crowd-sourced approach.}{Our GZX simulation processes a total of 932,017 visual classifications. From Section~\ref{sec: swap is faster} we know that GZ2 requires at least 35 votes per subject to retain galaxy classifications that are consistent 95\% of the time. At best, GZ2 could have retired 26,629 subjects with the classifications we process during our GZX run. This implies that we have increased the classification rate by at least a factor of 8, while requiring only 13\% as many human classifications. }
...
This represents a factor of \replaced{11.4}{at least 8} increase in the classification rate as well as
 nearly an order of magnitude reduction in human effort compared to the original GZ2 project. 
...
\begin{figure}[t!]
\includegraphics[width=7in]{GZX_quality_comparison.pdf}
...
\added{When applying GZX to future imaging programs, there will be no ``ground truth'' labels for comparison. In some sense, these thresholds can be interpreted as a prior for the SWAP \p~value, the initial probability for a subject to be \feat. As we show in Appendix~\ref{sec: tweaking swap}, changing the prior has little affect on the retirement rate but does result in considerable variability in the completeness and purity of the resulting classifications. The choice of whether to optimize SWAP to recover pure or complete samples is a decision for the science team. }
