...
\documentclass[twocolumn]{aastex6}%, trackchanges, linenumbers
...
\author{Melanie R. Beck\altaffilmark{1}, Claudia Scarlata\altaffilmark{1}, Lucy F. Fortson\altaffilmark{1}}
...
collected during the Galaxy Zoo 2 (GZ2) project\replaced{, reprocessed}{. We reprocess 
the top-level question of the GZ2 decision tree} with a classification aggregation 
algorithm dubbed SWAP, originally developed for the Space Warps gravitational 
lens project. Incorporating the SWAP algorithm \added{through a simplified binary classification scheme} increases the classification rate 
by a factor of 4.7, classifying 226,124 galaxies in 92 days of GZ2 project time\replaced{. 
GZ2 classified  $\sim$$48$K in the same time period.}{, during which GZ2 classified $\sim$$48$K. } \replaced{This increased rate does not 
diminish the quality of classification as we maintain}{The quality of these classifications remains high, with} 95.7\% accuracy as compared 
to GZ2 published data. We next combine this with a Random Forest machine 
...
210,543 galaxies in just 32 days of GZ2 project time. Again, this impressive 
performance is achieved with 93.5\% accuracy. As the Random Forest 
algorithm requires a minimal amount of computational cost, this result has
...
\added{In this paper we focus on the first question of the Galaxy Zoo 
decision tree.} \replaced{Our}{We demonstrate that our} method 
...
\M{20}\added{, well-suited for the top-level question of the GZ2 decision tree, 
as discussed below}. After a sufficient number of subjects have been classified 
...
{In order to compare our classification output with GZ2 we assign each subject a descriptive label. GZ2 classifications are composed of volunteer vote fractions ($f_{\mathrm{response}}$) for each response to every task in the decision tree, derived from the fraction of volunteers who voted for a particular response. GZ2 classifications are thus continuous. A common technique is to place a threshold on these vote fractions to select samples with an emphasis on purity or completeness, depending on the science case. For our current analysis we choose a threshold of 0.5, that is, if \ffeat+\fstar~$ >$ \fsmooth, the galaxy is labelled~\feat, otherwise it is labelled~\notfeat.  Though naive, we will demonstrate that this threshold produces adequate results, though a more sophistcated mechanism will be explored in a future publication.} 
 We note that only 512 subjects in the GZ2 catalog have a majority \fstar, contributing less than half a percent contamination \added{by combining the ``star or artifact" with ``features or disk" responses}.
...
Warps system.}
{Though we cannot retroactively train GZ2 volunteers, we develop a gold standard sample in lieu of simulated data, and arrange the classification order of gold standard data in order to mimic the Space Warps system.}
...
%% -------------------------------------------------------------------------------
\begin{figure}[t!]
\includegraphics[width=3.8in]{f2.pdf}
\caption{Confusion matrices for 1000 randomly selected GZ2 volunteers after fiducial SWAP assessment. Circle size is proportional to the number of gold standard subjects each volunteer classified. The histograms on top and right represent the distribution of each component of the confusion matrix for all volunteers.  A quarter of GZ2 volunteers are ``Astute"; \replaced{they are adept at correctly identifying both~\feat~and~\notfeat~subjects.}{they correctly identify both \feat~and \notfeat~subjects more than 50\% of the time.} The peaks at 0.5 in both distributions are due primarily to volunteers who see only one training image: only half of their confusion matrix is updated. \label{fig: volunteer training}}
\end{figure}
...
\added{This ensures that our expert labels are defined in exactly the same manner as the labels for all SDSS galaxies in the GZ2 sample.}
...
%%%-------------------------------------------------------
\begin{figure}[t!]
\includegraphics[width=3.35in]{f3.pdf}
...
For our fiducial  simulation, we initialize all confusion matrices at \added{(\Pf, \Pn) =} (0.5, 0.5), 
...
\added{We discuss these discrepancies section~\ref{sec: swap gz2 disagree}.}
...
\added{We explore these subjects in redshift, magnitude, physical size, and concentration. We find no correlation with any of these variables, suggesting there are no physical reasons why SWAP's label disagrees with \raw. This is not surprising given that GZ2 subjects were selected from the original Galaxy Zoo project to be precisely those subjects where visual classification was possible.}
...
subjects that have $0.4 \le$~\ffeat+\fstar~$\le 0.6$. \replaced{indicating that 
the GZ2 raw vote fractions are simply too uncertain to provide high quality labels.}{It is thus unsurprising that SWAP and GZ2 disagree most within the confidence interval of our selected GZ2 threshold.} 
...
volunteers get it wrong \added{compared to expert classifications}. These issues can be mitigated by requiring each subject reach 
...
\replaced{GZX thus provides an order of magnitude increase in the rate of classification
over the traditional crowd-sourced approach.}{Though our analysis considers the only the top-level task of GZ2's decision tree, GZX suggests a tantalizing potential to increase the classification rate by an order of magnitude over the traditional crowd-sourced approach.}
...
same information. \replaced{We argue that the machine classifications are valid 
and complimentary to human classifications.}{Both machine and human classifcations each provide valuable and complimentary information for identifying \feat~galaxies.}

\replaced{Of the 6127 subjects that were deemed false positives, i.e., galaxies retired by the 
...
A random sample is shown in Figure~\ref{fig: machine false pos}. \deleted{where the value 
in the lower left corner is the raw GZ2 featured vote fraction, \ffeat; 
the fraction of volunteers who classified that subject as \feat. 
...
Regarding the first point, the full sample of false positives has $\langle f_{\mathrm{smooth}} \rangle = 0.645 \pm 0.106$  with 56.7\%  having \fsmooth~$\le 0.65$. 
This indicates that volunteers have not reached a strong consensus for a majority
 of these subjects; behavior that could be modified by providing actual training images 
and live feedback as performed in \cite{Marshall2016}. 
We note that 71.7\% of these galaxies are labelled~\feat~after GZ2's debiasing process. }
{
That the machine strongly identifies these galaxies as \feat~(\pmachine~$\ge 0.9$) where humans instead classify them as \notfeat~(\ffeat~$\le 0.5$) has several contributing factors:
1) as discussed in Section~\ref{sec: swap gz2 disagree}, the threshold we chose carries with it a confidence interval such that subjects with $0.4 < $\ffeat+\fstar$ < 0.6$ are most likely to receive disagreeing labels from other classying agents,
2) the first task of the GZ2 decision tree asks a very specific question that 
does not necessarily correlate with a split between early- and late-type galaxies, and 
3) the machine learns on morphology diagnostics that are very different from visual inspection. 
}

\added{We find that 41.4\% of these false positves have $0.4\le$\ffeat+\fstar$<0.5$ indicating that the disagreement between humans and machine is likely due to the labels we assign at our given threshold. However, we also find that 43.5\% of false positives have \ffeat+\fstar$\le0.35$, where this discrepancy is not as easily explained. We examine a random sample of false positives in this regime in Figure~\ref{fig: machine false pos} where, for clarity, we display only the \ffeat~value. The majority of these objects are discs without defining features such as spiral arms or strong bars. Whether this is the reason the majority of volunteers classify these objects as ``smooth" is beyond the scope of this paper, however, this behavior might be modified by providing actual training images and live feedback as performed in \cite{Marshall2016}. We suggest that, at least for this particular question, if either human or machine identifies a subject as \feat~it's likely the subject is discy and worth further investigation. }
...
%% -------------------------------------------------------------------------------
\begin{figure*}[t]
\includegraphics[width=3.35in]{A1a.pdf}
\includegraphics[width=3.35in]{A1b.pdf}
...
%%%-------------------------------------------------------
\begin{figure*}[t!]
\includegraphics[width=3.08in]{A2a.pdf}
\includegraphics[width=3.7in]{A2b.pdf}
